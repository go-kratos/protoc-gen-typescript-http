...
func generateStreamInfra(f *codegen.File) {
	f.P("export interface Subscription {")
	f.P(t(1), "unsubscribe(): void;")
	f.P("}")
	f.P()
	f.P("export interface ServerStream<T> {")
	f.P(t(1), "subscribe(onData: (data: T) => void): Subscription;")
	...
	f.P(t(1), "private subscribers: Array<(data: T) => void> = [];")
	...
	f.P(t(4), "this.subscribers.forEach(fn => fn(data));")
	f.P(t(3), "} catch (err) {")
	...
	f.P()
	f.P(t(1), "subscribe(onData: (data: T) => void): Subscription {")
	f.P(t(2), "this.subscribers.push(onData);")
	f.P(t(2), "return {")
	f.P(t(3), "unsubscribe: () => {")
	f.P(t(4), "this.subscribers = this.subscribers.filter(fn => fn !== onData);")
	f.P(t(3), "},")
	f.P(t(2), "};")
	f.P(t(1), "}")
	f.P()
	...
	f.P(t(1), "private subscribers: Array<(data: TOut) => void> = [];")
	f.P(t(1), "private errorHandlers: Array<(error: Error) => void> = [];")
	f.P()
	f.P(t(1), "constructor(url: string) {")
	...
	f.P(t(1), "}")
	f.P()
	f.P(t(1), "subscribe(onData: (data: TOut) => void): Subscription {")
	f.P(t(2), "this.subscribers.push(onData);")
	f.P(t(2), "return {")
	f.P(t(3), "unsubscribe: () => {")
	f.P(t(4), "this.subscribers = this.subscribers.filter(fn => fn !== onData);")
